...
import { subscriptionLabels } from '@/lib/rules';
...
export default function SettingsPage() {
  ...
  const saveSettings = (event: FormEvent<HTMLFormElement>) => {
    ...
    updatePreferences({
      ...
      language: form.get('language') as 'fa' | 'en'
    });
  };
      ...
      <div className="grid cols-2">
        <section className="card">
          ...
          <form className="form" onSubmit={saveSettings}>
            ...
            <div className="form-row"><label className="label">Language</label><select className="select" name="language" defaultValue={currentUser.preferences.language}><option value="fa">Persian</option><option value="en">English</option></select></div>
            <button className="btn primary">Save Settings</button>
          </form>
        </section>
        <section className="card highlight">
          <h2>Current Subscription</h2>
          <p className="page-title" style={{ fontSize: 34 }}>{subscriptionLabels[currentUser.subscription]}</p>
      ...
      {upgradeOpen ? (
        <Modal title="Choose Subscription" onClose={() => setUpgradeOpen(false)}>
          <div className="grid cols-3">
            <button className="card" onClick={() => handleUpgrade('base')}><h3>Base</h3><p className="muted">Free, 60 daily streams, 6 playlists</p></button>
            <button className="card" onClick={() => handleUpgrade('silver')}><h3>Silver</h3><p className="muted">{formatCurrency(pricing.silver)} / month, downloads and profile image</p></button>
            <button className="card highlight" onClick={() => handleUpgrade('gold')}><h3>Gold</h3><p className="muted">{formatCurrency(pricing.gold)} / month, early access and analytics</p></button>
          </div>
        </Modal>
      ) : null}
    </AppShell>
